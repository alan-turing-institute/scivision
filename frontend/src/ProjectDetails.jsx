...
function Project() {
  ...
  for (const model_name of project.models) {
    ...
    let thumbnail = (
      <Link to={full_path}>
        <img
          src={model_thumbnails[`./${model.name}.jpg`]}
          className="halfsize_thumbnails"
          alt={model.name}
        />
      </Link>
    );
  ...
  for (const datasource_name of project.datasources) {
    ...
    const datasource = datasources.entries.find(
      (datasource) => datasource.name === datasource_name,
    );
    let thumbnail = (
      <Link to={full_path}>
        <img
          src={datasource_thumbnails[`./${datasource.name}.jpg`]}
          className="halfsize_thumbnails"
          alt={datasource.name}
        />
      </Link>
    );
    datasource_links.push(
      <OverlayTrigger
        key={datasource.name}
  ...
  return (
    <>
      ...
      <img
        src={project_thumbnails[`./${project.name}.jpg`]}
        alt={project.header}
      />
      <MarkdownView
