                      ...
                      (new_entry_submission) => {
                          ...
                          if (pr_flag) {
                              ...
                              octokit.createPullRequest({
                                  ...
                                  body: "This PR was automatically generated by the Scivision frontend",
                                  base: "main",
                                  head: `add-model-${entry.name}`,
                                  update: true,
                                  forceFork: false,
                                  changes: [
                                      {
                                          files: {
                                              "scivision/catalog/data/models.json":
                                              ({ exists, encoding, content}) => {
                                                  const content_str = Buffer.from(content, encoding);
                                                  let cat = JSON.parse(content_str);
                                                  cat.entries.push(entry);
                                          ...
                                          commit: `Create entry for ${entry.name} in the model catalog`
                                      }
                                  ]
                              });
                          } else {
                              download("one-model.json", JSON.stringify(entry, null, 2));
                          }
                      }
                  }
                  schema={model_schema}
